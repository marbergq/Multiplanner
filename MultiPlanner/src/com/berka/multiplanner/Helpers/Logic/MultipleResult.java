...
public class MultipleResult implements IResult {
	...
	static class SearchForAllLocations {
		...
		public void foundLocation(ILocation loc) {
			...
			for (int i = 0; i < this.from.size(); i++) {
				if (from.get(i).first.getLocationid().intValue() == loc
						.getLocationid().intValue()) {
					Pair<ILocation, Boolean> p = new Pair<ILocation, Boolean>(
							from.get(i).first, true);
					temp.add(p);
				} else {
					temp.add(from.get(i));
				}
	...
	public List<Trip> getTrips(Planner planner, Boolean onlySameTimeArrival,
			Boolean onlyOneResultPerStation) throws ParseException {
	...
	private String findTheLatestTimeInTrip(Trip trip) throws ParseException {
		...
		for (List<Segment> seg : trip.getSegments()) {
			if (Thread.currentThread().isInterrupted())
				return null;
			if (candidate == null) {
				candidate = TimeFilter.getDateFromString(seg
						.get(seg.size() - 1).getArrival().getDatetime());
				continue;
			}
			Calendar buff = TimeFilter.getDateFromString(seg
					.get(seg.size() - 1).getArrival().getDatetime());
	...
	private Set<Set<String>> getTripsWithinLimit(Planner planner)
			throws ParseException {
		...
		for (String key : getArrivalTimes()) {
			...
			for (String key2 : getArrivalTimes())
				if (TimeFilter.isTimeWithinRange(key, key2,
						planner.getAnkomstIntervall()))
	...
	public List<List<Segment>> getSegmentsForArrivalTime(String ArrivalTime) {
		if (!map.containsKey(ArrivalTime))
			throw new IllegalArgumentException(
					"This arrival time does not exist");
		else
			return map.get(ArrivalTime);
	}

	public HashMap<Mot, List<List<Segment>>> getSegmentsWithSameLastTravel(
			String ArrivalTimne) {
	...
	@Override
	public List<Trip> getSingleTripsResult(Planner planner)
			throws ParseException {
		// TODO Auto-generated method stub
		List<Trip> returnList = new ArrayList<Trip>();
		Set<String> availableTimes = TimeFilter.filterTrips(map, planner);
		for (String time : availableTimes) {
			Trip t = new Trip();
			t.addAllSegments(map.get(time));
			t.setAnkomstTid(findTheLatestTimeInTrip(t));
			returnList.add(t);
